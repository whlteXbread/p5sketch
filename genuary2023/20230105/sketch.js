...
function preload() {
  // load the shader.
  squareShader = loadShader("debug.vert", "debug.frag");
}
...
function draw() {
  ...
  for (let i = 0; i < numParticles; i++) {
    if (i == 10) {
      // + (2.5 * random())
      polygonCenterCoordsX.push(polygonCenterCoords[i][0] + (50. * random()));
      polygonCenterCoordsY.push(polygonCenterCoords[i][1] + (50. * random()));
      continue
    }
    // add a tiny amount of spice just because i guess
    polygonCenterCoordsX.push(polygonCenterCoords[i][0]);
    polygonCenterCoordsY.push(polygonCenterCoords[i][1]);
  }
